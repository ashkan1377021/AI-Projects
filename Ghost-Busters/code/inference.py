...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        beliefs = self.beliefs
        GhostsPositions = self.allPositions

        result = DiscreteDistribution()
        for prvPositions in GhostsPositions:
            newPosDist = self.getPositionDistribution(gameState, prvPositions)
            for newPosition, probability in newPosDist.items():
                result[newPosition] += beliefs[prvPositions] * probability
        self.beliefs = result
        # raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particlesNumber = self.numParticles
        legalPositions = self.legalPositions

        for _ in range(int(particlesNumber / len(legalPositions))):
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        copy = self.particles.copy()
        self.particles = []
        for particle in copy:
            self.particles.append(self.getPositionDistribution(gameState, particle).sample())
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in particles:
            ...
            for i in range(self.numGhosts):
                probability *= self.getObservationProb(observation[i], pacmanPos, particle[i], jailPos)
        ...
        for value in values:
            if value != 0:
                is_all_zero = False
        if not is_all_zero:
            self.particles = [distribution.sample() for none in range(self.numParticles)]
        else:
            self.initializeUniformly(gameState)
        # raiseNotDefined()
